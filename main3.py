...
def log_check(label: str, value, warn_if_none: bool = True):
    if value is None or value == {} or value == [] or value == "":
        if warn_if_none:
            logger.warning("CHECK | %-40s | EMPTY/NONE", label)
        else:
            logger.info("CHECK | %-40s | EMPTY (ok)", label)
    else:
        display = str(value)[:80]
        logger.info("CHECK | %-40s | OK | %s", label, display)
...
log_check("FOOTBALL_DATA_API_KEY", bool(FOOTBALL_DATA_API_KEY), warn_if_none=False)
...
NATIONALITY_FLAGS: dict = {
    ...
    "davidovich": "ES", "carreno": "ES", "badosa": "ES",
...
class ELOSystem:
    ...
    def _load(self, filepath: Path):
        ...
        if data:
            ...
            logger.info("ELO %s: no data yet (bootstrap needed)", self.sport)
    ...
    def update(self, name_a: str, name_b: str, score_a: float, is_home_a: bool = False):
    ...
    def predict(self, home: str, away: str, apply_home_advantage: bool = True) -> dict:
        ...
        if self.sport == "football":
            ...
            dp = draw_factor
            total = hp + ap + dp
            home_prob, away_prob, draw_prob = hp / total, ap / total, dp / total
        return {
...
class DataBootstrap:
    ...
    def should_run(self) -> bool:
        if FORCE_BOOTSTRAP:
            logger.info("Bootstrap: forced")
            return True
        if not CFG.BOOTSTRAP_FLAG.exists():
            logger.info("Bootstrap: first run")
            return True
        try:
            flag_time = datetime.fromisoformat(CFG.BOOTSTRAP_FLAG.read_text().strip())
            age_days = (datetime.now(timezone.utc) - flag_time).days
            if age_days >= 7:
                logger.info("Bootstrap: data %d days old - refreshing", age_days)
                return True
        except Exception:
            return True
    ...
    def run(self):
        ...
        log_check("Bootstrap football teams", len(self.elo_football.ratings))
        log_check("Bootstrap tennis players", len(self.elo_tennis.ratings))
    ...
    def _build_football_elo(self):
        ...
        for code, name in self.FOOTBALL_LEAGUES:
            ...
            for season in seasons:
                ...
                for _, row in df.iterrows():
                    try:
                        ...
                        score = 1.0 if ftr == "H" else (0.0 if ftr == "A" else 0.5)
            ...
            total += count
            if count:
                logger.info("Football ELO: %s -> %d matches", name, count)
    ...
    def _build_tennis_elo(self):
        ...
        for filename in self.TENNIS_FILES:
            ...
            total += count
            if count:
                logger.info("Tennis ELO: %s -> %d matches", filename, count)
            time.sleep(0.2)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if status == 429:
                        ...
                        logger.error("HTTP %d in %s", status, func.__name__)
...
def calculate_combined_ev(
    ...
    for market_key, market_data_list in markets_data.items():
        ...
        expected = CFG.MARKET_EXPECTED_OUTCOMES.get(market_key, {"min": 2})
        ...
        for outcome_name, sharp_data in sharp_odds.items():
            ...
            if elo_prediction and market_key == "h2h":
                ...
                if "draw" in name_l or "tie" in name_l:
                    ...
                    h_matches = elo_prediction.get("home_matches", 0)
                    a_matches = elo_prediction.get("away_matches", 0)
                    elo_diff = elo_prediction.get("elo_diff", 0)
                    if h_matches >= 5 and a_matches >= 5:
                        if elo_diff > 0:
            ...
            if ev > CFG.MAX_REALISTIC_EV:
                logger.warning("Rejected unrealistic EV=%.1f%% for %s", ev * 100, outcome_name)
                continue
        ...
        if best_opp:
            ...
            log_check(
                f"EV opp {market_key}",
                f"{best_opp['pick']} EV={best_opp['edge_pct']:.1f}% "
                f"odds={best_opp['odds']} elo={best_opp['elo_used']}",
            )
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.get(
                ...
                log_check("Odds API requests used this month", used)
                ...
                log_check("Events in 2h window", len(result))
                return result
    except Exception as e:
...
async def fetch_sofascore_stats_async(match_id: int) -> dict:
    ...
    for key, result in zip(endpoints.keys(), results):
        if not isinstance(result, Exception) and result is not None:
            data[key] = result
    log_check(f"SofaScore data for match {match_id}", list(data.keys()), warn_if_none=False)
    return data
...
class FootballDataAdapter:
    ...
    def __init__(self):
        ...
        try:
            if datetime.now(timezone.utc).date() > datetime.fromisoformat(last_ts).date():
                self.call_count = 0
                logger.info("Football-Data call counter reset")
        except Exception:
            self.call_count = 0
        log_check("Football-Data calls today", self.call_count, warn_if_none=False)
    ...
    def find_team_id(self, team_name: str) -> Optional[int]:
        ...
        if data and data.get("teams"):
            ...
            logger.info("FD team: %s -> %d", team_name, team_id)
        cache[key] = team_id
    ...
    def get_team_recent_form(self, team_id: int, team_name: str) -> dict:
        ...
        data = self._raw_get(f"/teams/{team_id}/matches",
                             {"status": "FINISHED", "limit": 5})
        if not data:
            return {}
        form = self._parse_form(data, team_id)
        log_check(f"Form {team_name[:20]}", form.get("form_string"), warn_if_none=False)
    ...
    def _parse_form(self, data: dict, team_id: int) -> dict:
        ...
        return {
            ...
            "btts_rate": round(sum(1 for a, b in zip(gs, gc) if a > 0 and b > 0) / total, 2),
            "over25_rate": round(sum(1 for a, b in zip(gs, gc) if a + b > 2.5) / total, 2),
    ...
    def get_h2h(self, team1_id: int, team2_id: int) -> dict:
        ...
        if CacheManager.is_valid(self.daily_cache, cache_key, CFG.TTL_H2H):
            return CacheManager.get(self.daily_cache, cache_key) or {}
        data = self._raw_get(f"/teams/{team1_id}/matches",
                             {"status": "FINISHED", "limit": 20})
        if not data:
            return {}
        h2h = [m for m in data.get("matches", [])
               if {m.get("homeTeam", {}).get("id"),
                   m.get("awayTeam", {}).get("id")} == {team1_id, team2_id}]
        result = self._parse_h2h(h2h, team1_id)
...
async def get_stats_async(
    ...
    stats = {
        ...
        "sofascore": {}, "elo": {}, "data_quality": "none",
    }
    ...
    if elo_pred and (elo_pred.get("home_matches", 0) >= 3 or
                     elo_pred.get("away_matches", 0) >= 3):
        stats["elo"] = elo_pred
        log_check(
            f"ELO {home[:12]} vs {away[:12]}",
            f"H={elo_pred['home_prob']:.1%} "
            f"D={elo_pred['draw_prob']:.1%} "
            f"A={elo_pred['away_prob']:.1%} "
            f"hm={elo_pred['home_matches']} am={elo_pred['away_matches']}",
            warn_if_none=False,
        )
    ...
    if coros:
        ...
        for name, result in zip(task_names, gathered):
            if isinstance(result, Exception):
                logger.warning("Stats error (%s): %s", name, result)
                continue
    ...
    log_check(f"Data quality {home[:12]} vs {away[:12]}", stats["data_quality"])
...
def calculate_confidence(
    ev_edge: float, stats: dict, market: str, has_sharp: bool
) -> tuple:
    score = 50
...
def build_stats_summary(stats: dict, home: str, away: str) -> str:
    ...
    if elo and elo.get("home_matches", 0) >= 3:
        parts.append(
            f"ELO: Home={elo['home_elo']:.0f} Away={elo['away_elo']:.0f} "
            f"Diff={elo['elo_diff']:.0f} | "
            f"Prob: H={elo['home_prob']:.1%} D={elo['draw_prob']:.1%} "
            f"A={elo['away_prob']:.1%} | "
            f"Matches: H={elo['home_matches']} A={elo['away_matches']}"
        )
    if hf:
        parts.append(
    ...
    if ss:
        parts.append(f"SOFASCORE: {json.dumps(ss, separators=(',', ':'))[:800]}")
...
def call_groq_sdk(
    ...
    if "qwen" not in model.lower() and "8b" not in model.lower():
        kwargs["response_format"] = {"type": "json_object"}
    try:
        res = groq_client.chat.completions.create(**kwargs)
        content = res.choices[0].message.content
        log_check(f"Groq {model[:25]}", content[:60] if content else None)
        return content
    except Exception as e:
...
def generate_dual_ai_analysis(
    ...
    default_response = {
        ...
        "logic": "Sharp market analysis confirms significant value on this selection.",
    }
    ...
    sys1 = (
        ...
        "RULES:\n"
        "- Use ONLY the statistics provided. Never invent numbers.\n"
        "- Never mention EV, models, algorithms, or data quality.\n"
        "- If no stats: state pick is driven by sharp market discrepancy.\n"
        "- Determine EXACT country flag emoji for home_flag and away_flag.\n"
    ...
    try:
        ...
        log_check("Model 1 result", "OK" if analysis_1 else "FAILED")
    except Exception as e:
    ...
    sys2 = (
        ...
        "Keep it under 2 sentences. Professional tipster tone.\n"
        "If stats are absent do NOT fabricate form or momentum data.\n"
    ...
    try:
        ...
        log_check("Model 2 result", "OK" if analysis_2 else "FAILED")
    except Exception as e:
    ...
    if analysis_1:
        ...
        if analysis_1.get("home_flag"):
            result["home_flag"] = validate_flag(analysis_1["home_flag"], home)
        if analysis_1.get("away_flag"):
            result["away_flag"] = validate_flag(analysis_1["away_flag"], away)

    safe_logic = str(initial_logic).strip()
    result["logic"] = safe_logic[:600] + "..." if len(safe_logic) > 600 else safe_logic

    log_check("Final confidence", result["confidence"])
    log_check("Final risk", result["risk_level"])
    log_check("Logic preview", result["logic"][:60])
...
def fetch_event_result(home: str, away: str) -> Optional[dict]:
    ...
    for event in res.json():
        if (event.get("home_team", "").lower() == home.lower() and
                event.get("away_team", "").lower() == away.lower() and
                event.get("completed")):
            return event
    return None
...
def check_and_report_results(sent_history: SentHistory) -> Optional[str]:
    ...
    for key, entry in pending:
        ...
        if not result_event:
            logger.info("Result not found yet: %s vs %s", home, away)
            continue
        ...
        try:
            hs = scores.get(home, {}).get("score", "?")
            aws = scores.get(away, {}).get("score", "?")
            result_str = f"{hs} - {aws}"
        except Exception:
            result_str = "? - ?"
        ...
        log_check(
            f"Result {home[:15]} vs {away[:15]}",
            f"score={result_str} won={won}",
            warn_if_none=False,
        )
    ...
    roi_vals = [w.get("odds", 1.0) - 1.0 for w in wins] + [-1.0] * len(losses)
    roi = sum(roi_vals) / len(roi_vals) if roi_vals else 0

    lines = ["\U0001F4CA <b>RESULTS REPORT</b>\n"]
    for w in wins:
        lines.append(
            f"\U00002705 <b>{html_lib.escape(w['home'])} vs "
    ...
    for l in losses:
        lines.append(
            f"\u274C <b>{html_lib.escape(l['home'])} vs "
            f"{html_lib.escape(l['away'])}</b>\n"
            f"   Pick: {html_lib.escape(l['pick'])} @ <code>{l['odds']}</code>\n"
            f"   Score: {l.get('result', '?')} \u2014 <b>LOSS</b>\n"
        )
    lines.append(
        f"\n\U0001F3AF <b>Session:</b> {len(wins)}W / {len(losses)}L | "
...
def send_telegram(message_html: str) -> bool:
    ...
    for chunk in chunks:
        try:
            ...
            if not res.ok:
                logger.error("Telegram error: %s - %s", res.status_code, res.text[:100])
                success = False
        except Exception as e:
...
async def async_main():
    log_section("ZBET90 ENTERPRISE ENGINE v3.1 STARTING")
    ...
    if not events:
        logger.info("No events in 2h window")
        return
    ...
    for event in events:
        ...
        logger.info("--- %s vs %s [%s]", home, away, sport)
        ...
        ai_data = generate_dual_ai_analysis(
            home, away, sport, display_pick, opp["market"],
        ...
        logic_escaped = html_lib.escape(
            str(ai_data.get("logic", "")).replace("<", "").replace(">", "")
        )

        msg = (
            f"{ai_data.get('sport_emoji', chr(0x1F3C6))} "
            f"<b>{html_lib.escape(sport)}</b>\n\n"
            f"\u2694\uFE0F <b>{html_lib.escape(home)}</b> "
            f"{ai_data.get('home_flag', chr(0x1F3F3) + chr(0xFE0F))}"
            f"  <b>vs</b>  "
            f"{ai_data.get('away_flag', chr(0x1F3F3) + chr(0xFE0F))} "
            f"<b>{html_lib.escape(away)}</b>\n"
            f"\u23F3 <b>Starts in:</b> {get_countdown_str(commence_time, now_utc)}\n\n"
            f"\U0001F3AF <b>PICK: {html_lib.escape(display_pick)}</b> "
            f"@ <code>{opp['odds']}</code>\n\n"
            f"\U0001F4CA <b>Market:</b> {html_lib.escape(opp['market_label'])}\n"
    ...
    log_check("Signals sent this run", total_sent, warn_if_none=False)
